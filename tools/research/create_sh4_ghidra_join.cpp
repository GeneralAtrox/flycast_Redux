...
{
	std::fprintf(stderr,
			"Usage: %s --output <join.json> --profile <profile.fcsh4profile> "
...
{
	std::filesystem::path output, profile, identity, replay, ghidraExport, program, script;
	std::uint64_t maximumBytes =
			research::DefaultMaximumSh4GhidraSemanticJoinBytes;
	for (int index = 1; index < argc; ++index)
	{
		const std::string argument = argv[index];
		if ((argument == "--output" || argument == "--profile"
	...
	if (output.empty() || profile.empty() || identity.empty() || replay.empty()
	...
	try
	{
		const auto summary = research::writeSh4GhidraSemanticJoin(output, profile,
				identity, replay, ghidraExport, program, script, maximumBytes);
		std::printf("CREATED flycast-research-sh4-ghidra-semantic-join-v1\n");
	...
	{
		std::fprintf(stderr, "FAILED semantic join creation: %s\n", exception.what());
		return 1;
	}
}
