...
$root = Join-Path ([IO.Path]::GetTempPath()) `
    ('flycast-research-gdrom-integration-' + [Guid]::NewGuid().ToString('N'))
...
try {
    ...
    Write-Output 'PASS flycast-research-gdrom-package-forced-abort-v1'
}
finally {
    ...
    if ($resolvedRoot.StartsWith($tempRoot,
            ...
            (Split-Path -Leaf $resolvedRoot) -match
                '^flycast-research-gdrom-integration-[0-9a-f]+$' -and
