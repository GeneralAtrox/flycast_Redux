			...
			case PCW::link:
				{
					...
					if (link->offset & 0x80000000)
					{
						...
						const u32 vramAddress = link->vramAddress & VRAM_MASK;
						memcpy(&vram[vramAddress], &mem_b[DMAC_SAR(2) & RAM_MASK], link->size);
						research::observePvrVramWrite(
								research::PvrVramWriteSource::Naomi2Elan,
								link->vramAddress, vramAddress, &vram[vramAddress],
								link->size, 0, sh4_sched_now64());
					...
					else if (link->offset & 0x20000000)
					{
						...
						DEBUG_LOG(PVR, "Texture DMA from eram %x -> %x (%x) %s", link->offset & ELAN_RAM_MASK, link->vramAddress & VRAM_MASK, link->size,
								data >= (u8 *)elanCmd && data < (u8 *)elanCmd + sizeof(elanCmd) ? "CMD" : "ERAM");
						const u32 vramAddress = link->vramAddress & VRAM_MASK;
						memcpy(&vram[vramAddress], &RAM[link->offset & ELAN_RAM_MASK], link->size);
						research::observePvrVramWrite(
								research::PvrVramWriteSource::Naomi2Elan,
								link->vramAddress, vramAddress, &vram[vramAddress],
								link->size, 0, sh4_sched_now64());
