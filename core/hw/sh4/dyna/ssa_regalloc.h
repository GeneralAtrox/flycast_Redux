...
private:
	...
	bool NeedsWriteBack(Sh4RegType reg, u32 version)
	{
		for (size_t i = opnum + 1; i < block->oplist.size(); i++)
		{
			...
			if (shilIsFullContextCall(op->op) || (mmu_enabled() && (op->op == shop_readm || op->op == shop_writem || op->op == shop_pref)))
				return true;
	...
	bool EarlyLoad(int cur_op, int early_op)
	{
		...
		if (shilIsFullContextCall(next_op->op) || next_op->op == shop_sync_sr || next_op->op == shop_sync_fpscr)
			return false;
